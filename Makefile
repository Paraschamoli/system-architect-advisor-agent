...
run: env-check ## Run the agent locally
	@echo "🤖 Starting System-Architect-Advisor-Agent..."
	@uv run python -m system_architect_advisor_agent

.PHONY: run-dev
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting System-Architect-Advisor-Agent in development mode..."
	@uv run watchfiles "python -m system_architect_advisor_agent" system_architect_advisor_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t system_architect_advisor_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t system_architect_advisor_agent:latest .

.PHONY: docker-run
docker-run: ## Run Docker container with .env file
	@echo "🐳 Running Docker container..."
	@docker run -d \
		--name system_architect_advisor_agent \
		-p 3773:3773 \
		--env-file .env \
		--restart unless-stopped \
		system_architect_advisor_agent:latest
...
docker-stop: ## Stop Docker container
	...
	@docker stop system_architect_advisor_agent || true
	@echo "✅ Container stopped"

.PHONY: docker-rm
docker-rm: docker-stop ## Remove Docker container
	@echo "🗑️  Removing Docker container..."
	@docker rm system_architect_advisor_agent || true
	@echo "✅ Container removed"

.PHONY: docker-logs
docker-logs: ## View Docker container logs
	@docker logs -f system_architect_advisor_agent

.PHONY: docker-shell
docker-shell: ## Open shell in running Docker container
	@docker exec -it system_architect_advisor_agent bash
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi system_architect_advisor_agent:latest || true
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep system_architect_advisor_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep system_architect_advisor_agent || echo "No images found"
	@echo "\nAgent Health:"
